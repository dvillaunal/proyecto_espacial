...
geo_df <- st_as_sf(geo_df, coords = c("X", "Y"))  %>%
...
mod_ols <- fit.variogram(variog2, model = vgm(1.48, 'Exp', 4.4336, kappa = 2.17), fit.method = 6)

```

##Error
```{r, include=FALSE, eval=FALSE}
pred_krig <- krige(geo_df$Temperature ~ X + Y, locations = geo_df,
      newdata = gdf, model = mod_ols)
```
...
ggplot(antioquia) +
  geom_sf() +
  geom_sf(data = geo_df, aes(col = Temperature))+
  theme_bw()
```

```{r}
geodist <- dist(as.data.frame(geo_df)[,5:6], diag = TRUE, upper = TRUE)
dist.temp <- dist(as.data.frame(geo_df)[,2], diag = TRUE, upper = TRUE)^2
mantel <- mantel(geodist, dist.temp)

normalidad <- shapiro.test(geo_df$Temperature)

test <- data.frame(Test = c('Shapiro-Wilk', 'Mantel'),
                   'p-value' = c(round(normalidad$p.value, 3),
                                 round(mantel$signif, 3)))
test %>% gt()
```

```{r}
variograma <- variogram(Temperature ~ X + Y, data = geo_df)

plot(variograma, main = 'Semivariograma empírico')
```
...
```{r}
ols <- fit.variogram(variograma, fit.method = 6,
                     model = vgm(model = c('Sph', 'Exp', 'Gau', 'Mat')))

plot(variograma, ols)
```

## Semivariograma Exponencial - RML 
```{r}
rml <- fit.variogram.reml(Temperature ~ X + Y, data = as_Spatial(geo_df),
                           model = vgm(1.48, 'Exp', 4.4336, kappa = 2.17))

plot(variograma, rml)
```

```{r}
grilla <- st_make_grid(x = antioquia, cellsize = 1) %>%
  st_centroid() %>%
  st_as_sf() %>%
  rename(geometry = x) %>%
  mutate(X = st_coordinates(.)[,1], Y= st_coordinates(.)[,2])

grafico <- function(bd, variable){
  ggplot(bd) +
...
```{r}
kriging.ols <- krige(geo_df$Temperature ~ X + Y, locations = geo_df,
      newdata = grilla, model = ols)
```

```{r}
res <- kriging.ols %>%
  st_intersection(antioquia)
...
```{r}
kriging.rml <- krige(geo_df$Temperature ~ X + Y, locations = geo_df,
      newdata = grilla, model = rml)
```
...
```{r}
grafico(res2, var1.pred)
grafico(res2, var1.var)
```
...
```{r}
cv.ols <- krige.cv(formula = Temperature~X+Y,
               locations = geo_df,
               model = ols,
               nfold = 10)
cv.ols
...
```{r}
cv.rml <- krige.cv(formula = Temperature~X+Y,
               locations = geo_df,
               model = rml,
               nfold = 10)
cv.rml
...
### RMSE
```{r}
round(sqrt(mean(cv.rml$residual^2)), 2)
```

### MAE
```{r}
round(mean(abs(cv.rml$residual)), 2)
```


## Referencias

-   https://unimedios.medellin.unal.edu.co/bitacora/conexion/1115-es-real-que-esta-haciendo-calor-en-medellin.html

-   https://www.eltiempo.com/colombia/medellin/por-que-aumenta-la-temperatura-en-medellin-76692 https://www.semana.com/impacto/articulo/bogota-registra-la-temperatura-mas-alta-en-su-historia/37055/https://minas.medellin.unal.edu.co/noticias/345-como-las-altas-temperaturas-en-las-ciudades-pueden-afectar-nuestra-salud

-   https://www.elcolombiano.com/antioquia/calor-en-medellin-septiembre-tuvo-el-dia-con-la-temperatura-mas-alta-NG11608849

-   https://noticias.caracoltv.com/antioquia/medellin-alcanzo-la-temperatura-mas-alta-de-los-ultimos-50-anos

-   https://siata.gov.co/sitio_web/index.php/noticia13

-   https://www.elmundo.com/noticia/Temporada-seca-propago-los-incendios-en-Medellin/377592

-   https://caracol.com.co/emisora/2020/01/17/medellin/1579281867_179798.html

-   https://www.metropol.gov.co/Paginas/Noticias/comenzo-la-segunda-temporada-de-disminucion-de-lluvias-en-el-valle-de-aburra.aspx

-   [[https://www.metropol.gov.co/Paginas/Noticias/comenzo-la-segunda-temporada-de-disminucion-de-lluvias-en-el-valle-de-aburra.aspx]{.underline}](https://www.metropol.gov.co/Paginas/Noticias/comenzo-la-segunda-temporada-de-disminucion-de-lluvias-en-el-valle-de-aburra.aspx)

-   [[https://caracol.com.co/emisora/2020/01/17/medellin/1579281867_179798.html]{.underline}](https://caracol.com.co/emisora/2020/01/17/medellin/1579281867_179798.html)

-   [[https://www.metropol.gov.co/Paginas/Noticias/influencia-del-cambio-climatico-en-precipitaciones-extremas-Antioquia.aspx]{.underline}](https://www.metropol.gov.co/Paginas/Noticias/influencia-del-cambio-climatico-en-precipitaciones-extremas-Antioquia.aspx)

-   chrome-extension://efaidnbmnnnibpcajpcglclefindmkaj/https://dssa.gov.co/images/2022/documentos/PACCSA.pdf
